...
<table width="100%" height="144" border="0" cellpadding="0" cellspacing="0" class="line_table">
    ...
    <tr>
        <td height="40" valign="top" colspan="2">
            <form id="searchItemForm" action="admin/listWebImage.do" method="post">
    ...
    <tr>
        <td height="102" valign="top" colspan="2">
            <c:if test="${webImageList==null}">
                <span class="vcher_null_tips">点击按钮查询，默认为查询所有记录！</span>
            </c:if>
            <c:if test="${webImageList!=null&&webImageList.size()=='0'}">
            ...
            <c:if test="${webImageList!=null&&webImageList.size()!='0'}">
                <table width="95%" cellpadding="1" cellspacing="0" class="vcher_list_table">
                    ...
                    <c:if test="${webImageList!=null&&webImageList.size()>0}">
                        <c:forEach var="webImage" items="${webImageList}">
                            <tr id="webImageItem_${webImage.id}">
                                <td>${webImage.id}</td>
                                <td>${webImage.type}</td>
                                <td>${webImage.title}</td>
                                <td>${webImage.createUser}</td>
                                <td>${webImage.createTime}</td>
                                <td>
                                    <a href="upload/${webImage.url}" title="点击查看大图" target="_blank"><img
                                            src="upload/${webImage.url}" alt="" width="218" height="100"/></a>
                                </td>
                                <td>&nbsp;<a
                                        href="javascript:delWebImage(${webImage.id})">删除</a></td>
                            </tr>
                        </c:forEach>
                    </c:if>
                    <tr>
